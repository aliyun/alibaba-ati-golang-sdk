...
func TestStandardDANEResolver_LookupTLSA_Found(t *testing.T) {
	...
	mux.HandleFunc(".", func(w dns.ResponseWriter, r *dns.Msg) {
		...
		m.AuthenticatedData = true // AD flag is no longer used; local validation checks RRSIGs
	...
	// DNSSECValid is false because the mock server does not return RRSIG records
	// for local DNSSEC chain validation.
	if result.DNSSECValid {
		t.Error("LookupTLSA() DNSSECValid = true, want false (no RRSIG in response)")
	}
...
func TestStandardDANEResolver_LookupTLSA_MultipleTLSA(t *testing.T) {
	...
	if result.DNSSECValid {
		t.Error("LookupTLSA() DNSSECValid = true, want false (no RRSIG records)")
	}
