...
func TestFqdn_ATIDiscoveryName(t *testing.T) {
	tests := []struct {
		name  string
		input string
		want  string
	}{
		{
			name:  "simple fqdn",
			input: "agent.example.com",
			want:  "_ati.agent.example.com",
		},
		{
			name:  "subdomain fqdn",
			input: "ote.agent.cs3p.com",
			want:  "_ati.ote.agent.cs3p.com",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := fqdn.ATIDiscoveryName()
			if got != tt.want {
				t.Errorf("ATIDiscoveryName() = %q, want %q", got, tt.want)
			}
		})
	}
}

func TestFqdn_IdentityTLSAName(t *testing.T) {
	tests := []struct {
		name  string
		input string
		want  string
	}{
		{
			name:  "simple fqdn",
			input: "agent.example.com",
			want:  "_ati-identity._tls.agent.example.com",
		},
		{
			name:  "subdomain fqdn",
			input: "ote.agent.cs3p.com",
			want:  "_ati-identity._tls.ote.agent.cs3p.com",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := fqdn.IdentityTLSAName()
			if got != tt.want {
				t.Errorf("IdentityTLSAName() = %q, want %q", got, tt.want)
			}
		})
	}
}
