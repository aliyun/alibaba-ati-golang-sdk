...
func TestNewClient(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name: "with base URL",
			opts: []Option{
				WithBaseURL("https://ra.ansagent.cn:8180/ans/api/v1"),
			},
			wantErr: false,
		},
		{
	...
	tests := []struct {
	...
	}{
		{
			...
			response: &models.RegistrationPending{
				Status:  "PENDING",
				ATIName: "test-agent.ati.example.com",
				AgentID: "test-id-123",
			},
			wantErr: false,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !tt.wantErr {
				...
				if result.ATIName != expected.ATIName {
					t.Errorf("ATIName = %v, want %v", result.ATIName, expected.ATIName)
				}
			}
		})
	}
...
func TestClient_ResolveAgent(t *testing.T) {
	...
	if result.ATIName != "ati://v1.0.0.test.example.com" {
		t.Errorf("ATIName = %q, want %q", result.ATIName, "ati://v1.0.0.test.example.com")
	}
}
