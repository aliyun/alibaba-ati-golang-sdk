...
func buildEventsCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "events",
		Short: "Retrieve paginated ATI events",
...
Examples:
  ati-cli events
  ati-cli events --limit 50
  ati-cli events --follow
  ati-cli events --follow --poll-interval 10`,
...
func pollEvents(ctx context.Context, c *registry.Client, params *eventsParams) error {
...
func printEvents(result *models.EventPageResponse) {
	...
	for i, event := range result.Items {
		fmt.Fprintf(os.Stdout, "\n%d. [%s] %s\n", i+1, event.EventType, event.ATIName)
