...
Examples:
  ati-cli revoke abc123 --reason KEY_COMPROMISE
  ati-cli revoke abc123 --reason SUPERSEDED --comments "Replaced by v2.0.0"`,
...
func runRevoke(agentID, reason, comments string) error {
	...
	if !models.IsValidRevocationReason(revocationReason) {
		return fmt.Errorf("invalid revocation reason: %s. See 'ati-cli revoke --help' for valid reasons", reason)
	}
