...
for use in agent registration.`,
	...
	cmd.Flags().StringVar(&csrVersion, "version", "", "Agent version for ATI URI (required, e.g., 1.0.0)")
...
func runGenerateCSRWithParams(host, org, country, version, outDir string, keySize int) error {
	...
	// Generate ATI URI
	atiURI := fmt.Sprintf("ati://v%s.%s", version, host)

	// Generate identity certificate CSR
	fmt.Fprintln(os.Stdout, "Generating identity certificate...")
	if err := generateCSR("identity", host, org, country, atiURI, keySize, outDir); err != nil {
	...
	if err := generateCSR("server", host, org, country, atiURI, keySize, outDir); err != nil {
	...
	fmt.Fprintf(os.Stdout, "  ati-cli register --name \"My Agent\" --host %s --version 1.0.0 \\\n", host)
...
func generateCSR(name, host, org, country, atiURI string, keySize int, outDir string) error {
	...
	// Parse ATI URI
	uri, err := url.Parse(atiURI)
	if err != nil {
		return fmt.Errorf("failed to parse ATI URI: %w", err)
	}
