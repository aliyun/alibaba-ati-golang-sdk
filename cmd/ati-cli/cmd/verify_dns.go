...
func buildVerifyDNSCmd() *cobra.Command {
	return &cobra.Command{
		...
		Long: `Verifies that all required DNS records (HTTPS, TLSA, _ans, _ati-badge) have been
...
func runVerifyDNS(_ *cobra.Command, args []string) error {
	...
	if cfg.JSON {
		...
		fmt.Fprintln(os.Stdout, "Use 'ati-cli status "+agentID+"' to view full details.")
		fmt.Fprintln(os.Stdout)
	}
