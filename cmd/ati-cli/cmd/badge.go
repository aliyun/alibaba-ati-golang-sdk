...
for a given agent ID. This includes the Merkle proof, payload data, and status.`,
	...
	cmd.Flags().StringVar(&transparencyBaseURL, "transparency-url", "", "Transparency log base URL (env: ATI_TRANSPARENCY_URL)")
...
func runBadgeWithParams(agentID string, auditTrail, checkpoint bool, transparencyBaseURL string) error {
	...
	if baseURL == "" {
		baseURL = os.Getenv("ATI_TRANSPARENCY_URL")
	}
	if baseURL == "" {
		// Derive from API base URL
		baseURL = strings.Replace(cfg.BaseURL, "ra.ansagent.cn:8180", "tl.ansagent.cn:8180", 1)
		baseURL = strings.Replace(baseURL, "ra.ansagent.cn:8180", "tl.ansagent.cn:8180", 1)
	}

	// Create Transparency Log client (no authentication needed - public endpoint)
	c, err := registry.NewTransparencyClient(
...
func outputBadgeJSON(ctx context.Context, c *registry.TransparencyClient, agentID string, logEntry *models.TransparencyLog, auditTrail, checkpoint bool) {
...
func outputBadgeHuman(ctx context.Context, c *registry.TransparencyClient, agentID string, logEntry *models.TransparencyLog, auditTrail, checkpoint bool) {
...
func printV1Payload(v1 *models.TransparencyLogV1) {
	...
	fmt.Fprintf(os.Stdout, "  ATI ID:          %s\n", event.ATIID)
	fmt.Fprintf(os.Stdout, "  ATI Name:        %s\n", event.ATIName)
...
func printEventMetadata(metadata *models.EventMetadata) {
	...
	if len(metadata.ATICapabilities) > 0 {
		fmt.Fprintf(os.Stdout, "    Capabilities:  %v\n", metadata.ATICapabilities)
	}
}
...
func printRawPayload(payload map[string]any) {
	...
	if atiName, ok := payload["ansName"].(string); ok {
		fmt.Fprintf(os.Stdout, "  ATI Name:        %s\n", atiName)
	}
	if atiID, ok := payload["ansId"].(string); ok {
		fmt.Fprintf(os.Stdout, "  Agent ID:        %s\n", atiID)
	}
